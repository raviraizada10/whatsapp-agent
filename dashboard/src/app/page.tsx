...
import { LayoutDashboard, Users, CalendarClock, Settings, MessageSquare, Plus, Trash2, Edit2, Loader2, BellRing, BellOff, Send, Sparkles, CheckCircle, XCircle, Clock } from 'lucide-react';
...
function OverviewTab({ schedules, contacts }: { schedules: any[], contacts: any[] }) {
  ...
  return (
    <div className="space-y-8">
      <h2 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400">Dashboard Overview</h2>
      ...
      <div className="mt-8 p-6 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-md">
        <h3 className="text-xl font-semibold mb-6">Active Executions</h3>
        <ul className="space-y-4">
          {activeSchedules.length === 0 ? <p className="text-slate-400">No active schedules yet.</p> : null}
          {activeSchedules.map(task => (
             <li key={task.id} className="flex items-center justify-between p-5 rounded-2xl bg-black/20 border border-white/5 hover:bg-white/5 transition duration-300">
             <div className="flex-1">
               <p className="font-bold text-white text-xl flex items-center gap-3">
                 {task.recipient_name}
                 <span className="px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-widest bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">Active</span>
               </p>
             ...
             <div>
                <button onClick={() => triggerNow(task.id)} title="Send Message Immediately" className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500 hover:text-white transition font-bold text-sm shadow-lg shadow-emerald-500/10">
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-400 mt-1">Configure global agent preferences.</p>
      </div>

      <div className="mt-8 grid gap-4">
        <div className="flex items-center justify-between p-6 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-md hover:border-emerald-500/30 transition-all duration-300">
          <div className="flex items-start gap-4">
             <div className={`p-3 rounded-xl shadow-lg border ${enabled ? 'bg-emerald-500/20 border-emerald-500/30 shadow-emerald-500/20 text-emerald-400' : 'bg-slate-800/50 border-white/10 text-slate-400'}`}>
                {enabled ? <BellRing className="w-6 h-6" /> : <BellOff className="w-6 h-6" />}
             </div>
             <div>
               <h3 className="text-xl font-bold text-white">Admin Forwarding</h3>
               <p className="text-slate-400 mt-1 max-w-md">When enabled, any reply received by your bot (from any contact) will automatically be forwarded to your personal admin number.</p>
             </div>
          </div>
          <button 
             onClick={onToggle} 
             className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors ${enabled ? 'bg-emerald-500' : 'bg-slate-600'}`}
          >
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage all your agent recipients here.</p>
        </div>
      ...
      <AnimatePresence>
        {showAdd && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md">
             <div className="flex flex-col gap-3">
                <div className="flex gap-4">
                  <input type="text" placeholder="Name (e.g. Dad)" value={name} onChange={e => setName(e.target.value)} className="bg-black/40 border border-white/10 px-4 py-2 rounded-xl text-white outline-none focus:border-emerald-500 flex-1" />
                  <input type="text" placeholder="Phone (e.g. 15551234567)" value={phone} onChange={e => setPhone(e.target.value)} className="bg-black/40 border border-white/10 px-4 py-2 rounded-xl text-white outline-none focus:border-emerald-500 flex-1" />
                </div>
                <textarea rows={2} placeholder="Global Persona Context — optional (e.g. He is very sarcastic and loves cricket, use casual language...)" value={persona} onChange={e => setPersona(e.target.value)} className="w-full bg-emerald-500/5 border border-emerald-500/20 px-4 py-2 rounded-xl text-emerald-300 placeholder:text-emerald-500/30 outline-none focus:border-emerald-500/50 resize-none text-sm" />
                ...
                <div className="flex justify-end gap-2">
                   <button onClick={() => { setShowAdd(false); setName(''); setPhone(''); setPersona(''); }} className="px-5 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 transition font-medium">Cancel</button>
      ...
      <div className="grid gap-4 mt-8">
        ...
        {contacts.map(c => (
          <div key={c.id} className="rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md hover:border-white/20 transition-all duration-300 overflow-hidden">
            {editingId === c.id ? (
              <div className="p-5 flex flex-col gap-3">
                <p className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Editing Contact</p>
                <div className="flex gap-3">
                  <input value={editName} onChange={e => setEditName(e.target.value)} placeholder="Name" className="bg-black/40 border border-white/10 px-4 py-2 rounded-xl text-white outline-none focus:border-emerald-500 flex-1 text-sm" />
                  <input value={editPhone} onChange={e => setEditPhone(e.target.value)} placeholder="Phone" className="bg-black/40 border border-white/10 px-4 py-2 rounded-xl text-white outline-none focus:border-emerald-500 flex-1 font-mono text-sm" />
                </div>
                <textarea rows={2} value={editPersona} onChange={e => setEditPersona(e.target.value)} placeholder="Global Persona Context (optional)" className="w-full bg-emerald-500/5 border border-emerald-500/20 px-4 py-2 rounded-xl text-emerald-300 placeholder:text-emerald-500/30 outline-none focus:border-emerald-500/50 resize-none text-sm" />
                <PersonaSuggestions onSelect={s => setEditPersona(s)} />
                <div className="flex justify-end gap-2">
                  <button onClick={() => setEditingId(null)} className="px-5 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-slate-300 transition text-sm font-medium">Cancel</button>
              ...
              <div className="flex items-center justify-between p-5 group">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mt-0.5 bg-black/30 px-2 py-0.5 rounded-md font-mono inline-block">+{c.phone}</p>
                    {c.persona_context && (
                       <p className="text-emerald-500/70 text-xs mt-2 max-w-xl italic border-l-2 border-emerald-500/30 pl-2">"{c.persona_context}"</p>
                    )}
                  </div>
                </div>
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => startEdit(c)} className="p-2.5 rounded-xl bg-white/5 hover:bg-blue-500/20 text-slate-300 hover:text-blue-400 transition cursor-pointer"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => handleDelete(c.id)} className="p-2.5 rounded-xl bg-white/5 hover:bg-red-500/20 text-slate-300 hover:text-red-400 transition cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            )}
          </div>
...
function StatCard({ title, value, trend }: { title: string, value: string, trend: string }) {
  return (
    <div className="p-6 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/10 hover:shadow-xl hover:shadow-black/20 transition-all duration-300">
      ...
      <p className="text-sm text-emerald-400 mt-3 font-medium flex items-center gap-1">{trend}</p>
    </div>
  );
}
